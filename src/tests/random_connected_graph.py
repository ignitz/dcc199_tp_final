    ...
    if not args.no_output:
        ...
        if args.pretty:
            ...
            for x in graph.edges:
                capacity = random.randrange(1, 999)
                if x[0] != len(nodes)-1 and x[1] != 0 and x[0] + 1 != x[1]:
                    a = random.randrange(0, 2)
                    if a == 0:
                        b = 1
                    else:
                        b = 0
                    temp = str(x[a]) + ' ' + str(x[b]) + ' ' + str(capacity) + '\n'
                    response += temp
                    m += 1
            for i in range(len(nodes)-1):
                capacity = random.randrange(1, 999)
                temp = str(i) + ' ' + str(i+1) + ' ' + str(capacity) + '\n'
                response += temp
                m += 1
            result = str(0) + ' ' + str(len(nodes)-1) +' ' + str(m) + '\n'
            result += response
            print(result)
